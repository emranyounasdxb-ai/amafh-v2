...
export function CasePage({ path, onNavigate }: { path: string; onNavigate: (path: string) => void }) {
  ...
  const columns: DataTableColumn<ApplicationRecord>[] = [
    { id: 'number', header: 'Case / Application Number', value: record => record.caseNumber ?? record.id, width: 200 },
  ...
  if (!selectedId) return <Workspace title="Applications / Cases" description={apiMode ? 'Customer-linked application records.' : 'Customer-linked local application records.'} breadcrumb={<Breadcrumb items={[{ label: 'Applications / Cases' }]} />} actions={allowed('create') && <Button onClick={() => onNavigate('/applications/new')}>Create Application</Button>}>
    {actorPicker}{!loading && !error && !canViewCases ? <WorkspaceState kind="permission" title="Permission required" description={apiMode ? 'Your User Type needs Cases: view permission.' : 'Select an acting user whose User Type has Cases: view permission in Administration → Permissions.'} /> : <DataTable tableId="application-cases" caption="Applications / Cases" columns={columns} rows={cases} rowId={record => record.id} state={loading ? 'loading' : error ? 'error' : 'ready'} error={error} onRetry={() => { void reload() }} onRefresh={() => { void reload() }} pageSize={10} filter={(record, query) => [record.caseNumber, record.id, record.bank, record.product, record.productVariant, record.stage, record.status, customers.find(customer => customer.id === record.customerId)?.type ?? '', customers.find(customer => customer.id === record.customerId) ? customerName(customers.find(customer => customer.id === record.customerId)!) : '', userName(record.caseOwnerId)].some(text => text.toLowerCase().includes(query.toLowerCase()))} filters={record => (statusFilter === 'all' || record.status === statusFilter) && (stageFilter === 'all' || record.stage === stageFilter)} filterControls={<div className="amafh-case-filters"><label>Status <select value={statusFilter} onChange={event => setStatusFilter(event.target.value)}><option value="all">All statuses</option>{['Pending SM Approval', 'SM Approved', 'SM Rejected'].map(state => <option key={state}>{state}</option>)}</select></label><label>Stage <select value={stageFilter} onChange={event => setStageFilter(event.target.value)}><option value="all">All stages</option>{[...new Set(cases.map(record => record.stage))].map(stage => <option key={stage}>{stage}</option>)}</select></label></div>} rowActions={record => <Button size="compact" variant="secondary" onClick={() => onNavigate(`/cases/${encodeURIComponent(record.id)}`)}>View</Button>} />}
  </Workspace>
  ...
  return <Workspace title="Case workspace" description={item.caseNumber} breadcrumb={<Breadcrumb items={[{ label: 'Applications / Cases', href: '/cases' }, { label: item.caseNumber }]} />} actions={<Button variant="secondary" onClick={() => onNavigate('/cases')}>Back to Cases</Button>}>
    {actorPicker}{actionError && <p role="alert" className="amafh-case-error">{actionError}</p>}
    <DetailSection title="Case Summary" items={[{ label: 'Case / Application Number', value: item.caseNumber }, { label: 'Status', value: item.status }, { label: 'Current Stage', value: item.stage }, { label: 'Locked', value: locked ? 'Locked · original submission protected' : 'Unlocked' }, { label: 'Created Date / Time', value: date(item.createdAt) }]} />
