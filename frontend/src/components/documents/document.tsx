...
export function DocumentViewer({ title, pdfUrl, status, tone = 'neutral', metadata, onDownload, onPrint, onSend, actions }: DocumentViewerProps) { return <Card className="amafh-document-viewer"><div className="amafh-document-viewer__header"><div><h2 className="amafh-h4">{title}</h2>{metadata && <div className="amafh-document-viewer__metadata">{metadata}</div>}</div>{status && <Badge tone={tone}>{status}</Badge>}</div><div className="amafh-document-viewer__actions">{onDownload && <Button variant="outline" onClick={onDownload} icon={<Download size={16} aria-hidden="true" />}>Download</Button>}{onPrint && <Button variant="outline" onClick={onPrint} icon={<Printer size={16} aria-hidden="true" />}>Print</Button>}{onSend && <Button variant="outline" onClick={onSend} icon={<Send size={16} aria-hidden="true" />}>Send</Button>}{actions}</div>{pdfUrl ? <iframe className="amafh-document-viewer__preview" title={`Preview: ${title}`} src={pdfUrl} /> : <WorkspaceState kind="empty" title="Preview unavailable" description="Download the document to view it." />}</Card> }
