...
import { ChevronDown, ChevronLeft, ChevronRight, Columns3, Download, GripVertical, Pin, RefreshCw, RotateCcw } from 'lucide-react'
...
export function DataTable<T>({ tableId, columns, rows, rowId, state = 'ready', error, onRetry, pageSize = 10, preferenceAdapter = localTablePreferences, onSelectionChange, bulkActions, filter, filters, filterControls, activeFilters, rowActions, renderExpanded, caption, savedViews, onCreateSavedView, onExport, onRefresh, freshness }: DataTableProps<T>) {
  ...
  return <section className="amafh-table-system" aria-label={caption ?? 'Data table'}>
    <div className="amafh-table-toolbar">
      {savedViews && savedViews.length > 0 && <Dropdown label="Saved views" trigger={<button type="button" className="amafh-table-saved-view" aria-label={`Saved view: ${activeView?.label ?? savedViews[0].label}`}><span>{activeView?.label ?? savedViews[0].label}</span><ChevronDown size={16} aria-hidden="true" strokeWidth={1.75} /></button>} items={[...savedViews.map(view => ({ id: view.id, label: view.label, description: view.scope === 'default' ? 'Default' : view.scope === 'personal' ? 'Personal' : 'Shared', onSelect: () => { setActiveViewId(view.id); if (view.preferences) setPreferences(reconcilePreferences(view.preferences, columns)); setPage(1); setSelected([]); setAnnouncement(`${view.label} view selected`) } })), ...(onCreateSavedView ? [{ id: 'create-view', label: 'Create saved view', onSelect: () => onCreateSavedView(preferences) }] : [])]} />}
      ...
      <div className="amafh-table-toolbar__actions">
        {filterControls}
        <span className="amafh-table-advanced"><SelectField label="Density" value={preferences.density} onValueChange={value => update({ density: value as TableDensity })} options={[{ value: 'compact', label: 'Compact' }, { value: 'default', label: 'Default' }, { value: 'comfortable', label: 'Comfortable' }]} /></span>
        <Button variant="secondary" icon={<Columns3 size={16} aria-hidden="true" strokeWidth={1.75} />} aria-expanded={columnsOpen} aria-controls={`${tableId}-columns`} onClick={() => setColumnsOpen(open => !open)}>Columns</Button>
        {onExport && <span className="amafh-table-advanced"><Button variant="outline" icon={<Download size={16} strokeWidth={1.75} aria-hidden="true" />} onClick={onExport}>Export</Button></span>}
        {onRefresh && <span className="amafh-table-advanced"><Button variant="outline" icon={<RefreshCw size={16} strokeWidth={1.75} aria-hidden="true" />} onClick={onRefresh}>Refresh</Button></span>}
        <span className="amafh-table-advanced"><Button variant="outline" icon={<RotateCcw size={16} aria-hidden="true" strokeWidth={1.75} />} onClick={reset}>Reset</Button></span>
        <span className="amafh-table-more"><Dropdown label="More table actions" trigger={<Button variant="secondary" aria-label="More table actions" icon={<ChevronDown size={16} aria-hidden="true" strokeWidth={1.75} />}>More</Button>} items={[{ id: 'compact', label: 'Compact density', onSelect: () => update({ density: 'compact' }) }, { id: 'default', label: 'Default density', onSelect: () => update({ density: 'default' }) }, { id: 'comfortable', label: 'Comfortable density', onSelect: () => update({ density: 'comfortable' }) }, ...(onExport ? [{ id: 'export', label: 'Export', onSelect: onExport }] : []), ...(onRefresh ? [{ id: 'refresh', label: 'Refresh', onSelect: onRefresh }] : []), { id: 'reset', label: 'Reset', onSelect: reset }]} /></span>
      </div>
    </div>
    ...
    {columnsOpen && <div className="amafh-table-column-panel" id={`${tableId}-columns`}>
      ...
      {ordered.map(column => <div className="amafh-table-column-option" key={column.id}>
        ...
        <SelectField label={`Pin ${column.header}`} labelHidden value={preferences.pinned[column.id] ?? 'none'} disabled={column.pinnable === false} onValueChange={value => update({ pinned: { ...preferences.pinned, [column.id]: value === 'none' ? null : value as PinSide } })} options={[{ value: 'none', label: 'No pin' }, { value: 'left', label: 'Pin left' }, { value: 'right', label: 'Pin right' }]} />
      </div>)}
    </div>}
    ...
    {state === 'loading' ? <WorkspaceState kind="loading" title="Loading records" /> : state === 'error' ? <WorkspaceState kind="error" title="Could not load records" description={error} action={onRetry && <Button onClick={onRetry}>Try again</Button>} /> : rows.length === 0 ? <WorkspaceState kind="empty" title="Nothing here yet" description="No records are available." /> : sorted.length === 0 ? <WorkspaceState kind="no-results" title="No results" description="Try a different search." action={<Button variant="outline" onClick={() => setQuery('')}>Clear search</Button>} /> : <>
          ...
          <thead><tr><th className="amafh-data-table__selection" scope="col"><input ref={selectAllRef} type="checkbox" aria-label="Select all rows on this page" checked={allSelected} onChange={event => setSelected(current => event.target.checked ? [...new Set([...current, ...pageIds])] : current.filter(id => !pageIds.includes(id)))} /></th>
            {visible.map(column => <th key={column.id} scope="col" aria-sort={preferences.sort?.id === column.id ? preferences.sort.direction === 'asc' ? 'ascending' : 'descending' : undefined} style={stickyFor(column)} draggable={!column.fixed} onDragStart={() => { dragColumnId.current = column.id }} onDragOver={event => event.preventDefault()} onDrop={event => { event.preventDefault(); if (dragColumnId.current) moveColumn(dragColumnId.current, column.id) }} onDragEnd={() => { dragColumnId.current = null }}>
              <div className="amafh-data-table__heading">
                {!column.fixed && <GripVertical size={14} aria-label={`Drag ${column.header} to reorder`} strokeWidth={1.75} />}
                ...
                {preferences.pinned[column.id] && <Pin size={12} aria-label={`Pinned ${preferences.pinned[column.id]}`} strokeWidth={1.75} />}
      ...
      <div className="amafh-table-pagination"><span>{sorted.length} results · Page {currentPage} of {pages}</span><div><Button variant="outline" size="compact" disabled={currentPage === 1} onClick={() => setPage(currentPage - 1)} icon={<ChevronLeft size={14} aria-hidden="true" strokeWidth={1.75} />}>Previous</Button><Button variant="outline" size="compact" disabled={currentPage === pages} onClick={() => setPage(currentPage + 1)} icon={<ChevronRight size={14} aria-hidden="true" strokeWidth={1.75} />}>Next</Button></div></div>
    </>}
