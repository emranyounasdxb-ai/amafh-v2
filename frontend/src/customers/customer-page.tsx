...
export function CustomerPage({ route, onNavigate, customersRepository = apiMode ? apiCustomerRepository : mockCustomerRepository, applicationsRepository = apiMode ? apiApplicationRepository : mockApplicationRepository }: { route: CustomerRoute; onNavigate: (path: string) => void; customersRepository?: CustomerRepository; applicationsRepository?: ApplicationRepository }) {
  ...
  const columns = useMemo<DataTableColumn<CustomerRecord>[]>(() => [
    ...
    { id: 'applications', header: 'Applications/Cases', value: item => applications.filter(application => application.customerId === item.id).length, width: 170 },
  ], [applications])
  if (route.mode === 'list') return <Workspace title={title} description="Independent customer master records" breadcrumb={<Breadcrumb items={breadcrumb} />} actions={preview.can('Cases', 'create') && <Button onClick={() => onNavigate('/applications/new')}>Create Application</Button>}><PermissionActor preview={preview} /><PermissionGate preview={preview} domain="Customers" action="view"><DataTable tableId="customers" caption="Customers" columns={columns} rows={customers} rowId={item => item.id} state={loading ? 'loading' : error ? 'error' : 'ready'} error={error} onRetry={() => { void reload() }} onRefresh={() => { void reload() }} filters={item => (typeFilter === 'all' || item.type === typeFilter) && (statusFilter === 'all' || item.status === statusFilter)} filterControls={<div className="amafh-customer-filters"><label>Customer Type <select value={typeFilter} onChange={event => setTypeFilter(event.target.value)}><option value="all">All types</option><option value="individual">Individual</option><option value="company">Company / Business</option></select></label><label>Status <select value={statusFilter} onChange={event => setStatusFilter(event.target.value)}><option value="all">All statuses</option><option value="active">Active</option><option value="inactive">Inactive</option></select></label></div>} rowActions={item => <Button variant="secondary" size="compact" onClick={() => onNavigate(detailPath(item.id))}>View</Button>} /></PermissionGate></Workspace>
