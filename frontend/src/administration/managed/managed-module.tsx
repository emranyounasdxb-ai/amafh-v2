...
export function ManagedModule({ config, route, onNavigate, repository }: { config: ManagedConfig; route: ManagedRoute; onNavigate: (path: string) => void; repository: ManagedRepository }) {
  ...
  return <div className="amafh-managed-module">{nav}{route.mode === 'list' ? <>{route.kind === null && <div className="amafh-managed-create">{config.kinds.map(kind => <Button key={kind.id} variant="secondary" onClick={() => onNavigate(`${base}/${kind.id}/new`)}>Create {kind.singular}</Button>)}</div>}<DataTable tableId={`${config.slug}-${route.kind ?? 'all'}`} caption={route.kind ? config.kinds.find(item => item.id === route.kind)?.label : config.label} columns={columns} rows={records.filter(item => !route.kind || item.kind === route.kind)} rowId={item => item.id} state={loading ? 'loading' : error ? 'error' : 'ready'} error={error} onRetry={() => { void reload() }} onRefresh={() => { void reload() }} filters={item => (statusFilter === 'all' || item.status === statusFilter) && (kindFilter === 'all' || item.kind === kindFilter)} filterControls={<div className="amafh-managed-filters"><label>Status <select value={statusFilter} onChange={event => setStatusFilter(event.target.value)}><option value="all">All statuses</option><option value="active">Active</option><option value="inactive">Inactive</option></select></label>{config.kinds.length > 1 && !route.kind && <label>Type <select value={kindFilter} onChange={event => setKindFilter(event.target.value)}><option value="all">All types</option>{config.kinds.map(kind => <option key={kind.id} value={kind.id}>{kind.label}</option>)}</select></label>}</div>} rowActions={item => <Button variant="secondary" size="compact" onClick={() => onNavigate(detailPath(config, item))}>View</Button>} /></>
