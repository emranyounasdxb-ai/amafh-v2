...
export function AuditModule({ route, onNavigate, repository = mockAuditRepository }: { route: AuditRoute; onNavigate: (path: string) => void; repository?: AuditRepository }) {
  ...
  if (route.mode === 'list') return <div className="amafh-managed-module"><FormSection title={repository === mockAuditRepository ? 'Local audit samples' : 'Audit records'} description={repository === mockAuditRepository ? 'Illustrative records only.' : 'Read-only records of application and security changes.'}><span>{repository === mockAuditRepository ? 'Read-only sample entries' : 'Read-only entries'}</span></FormSection><DataTable tableId="administration-audit-log" caption="Audit Log" columns={columns} rows={entries} rowId={item => item.id} state={loading ? 'loading' : error ? 'error' : 'ready'} error={error} onRetry={() => { void reload() }} onRefresh={() => { void reload() }} filters={item => sourceFilter === 'all' || item.source === sourceFilter} filterControls={<label className="amafh-managed-filters">Source <select value={sourceFilter} onChange={event => setSourceFilter(event.target.value)}><option value="all">All sources</option>{[...new Set(entries.map(item => item.source))].map(source => <option key={source} value={source}>{source}</option>)}</select></label>} rowActions={item => <Button variant="secondary" size="compact" onClick={() => onNavigate(`/administration/audit-log/${encodeURIComponent(item.id)}`)}>View</Button>} /></div>
